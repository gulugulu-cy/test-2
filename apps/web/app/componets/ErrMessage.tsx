export function ErrMessage(err_code: number, language: 'chinese' | 'english' | 'japanese', region: number) {
  const relogin = (
    <a
      className="underline"
      style={{ color: "#0070f0" }}
      ...
      target="_blank"
    >
      {language === 'chinese' ? "重新登录" : language === 'japanese' ? "再ログインする" : "log in again"}
    </a>
  );
  const gw = (
    <a
  ...
  switch (err_code) {
    case -10001:
      return language === 'chinese' ? (
        <div className="font-bold">账户凭证丢失，请 {relogin}</div>
      ) : language === 'japanese' ? (
        <div className="font-bold">アカウント証明が失われました。{relogin}してください。</div>
      ) : (
        <div className="font-bold">Account credentials lost, Please {relogin}</div>
      );
    case -10002:
      return language === 'chinese' ? (
    ...
    case -10005:
      return language === 'chinese' ? (
        <div className="font-bold">账户凭证过期，请 {relogin}</div>
      ) : language === 'japanese' ? (
        <div className="font-bold">アカウントの証明が期限切れです。{relogin}してください。</div>
      ) : (
        <div className="font-bold">Account credentials expired, Please {relogin}</div>
      );
    case -10006:
      return language === 'chinese' ? (
